...
var InitTempSetPage = function() {
    ...
    var initSearchBtn = function() {
        $('#btnSearch').on('click', function() {
            ...
            Util.setAjax(Inter.getApiUrl().getSalaryTemp, postData, function(data) {
                if (data.success) {
                    // var data={};
                    // data.data = { "type": "1", "basic": ["身份证/护照号", "姓名"], "fund": ["年度", "月度", "实发合计", "扣款合计"], "salary": ["年度", ], "salaryUrl": "http://hiring.oss-cn-beijing.aliyuncs.com/trusdom/salarymodel/%E5%B7%A5%E8%B5%84%E5%8D%95%E6%A8%A1%E6%9D%BF2003%E7%89%88.xls" }
                    var defaultTemp = com_Conf.getSalaryConf('tempItems'),
                        fixedItems = com_Conf.getSalaryConf('fixedItems'),
                        tempItems = $.extend({}, defaultTemp, data, true),
                    ...
                    for (var key in tempItems) {
                        ...
                        for (var i = 0; i < tempItems[key].length; i++) {
                            ...
                            if (!pageData.objItems[key]) {
                                pageData.objItems[key] = {};
                            }
                            pageData.objItems[key][tempItems[key][i]] = 1;
    ...
    var initAddBtn = function() {
        $('[name="addItem"]').on('click', function(e) {
            ...
            if (inputV && !pageData.objItems[inputV]) {
                var $virualDom = $(Util.template(strInputTemp, { column: inputV, hideCls: '',itemType: domFor }));
    ...
    var initSaveBtn = function() {
        $('#btnSave').on('click', function(e) {
            ...
            postData = $.extend({}, { salary: [], fund: [] }, postData);
            // console.log(postData);
            Util.setAjax(Inter.getApiUrl().saveSalaryTemp, postData, function(json) {
                if(json.success){
                    bootbox.alert("保存成功");
                }else{
                    bootbox.alert(jsonf.errMsg);
                }
            }, function(data) {
                // console.log(data);
            });
    ...
    var initPositionHandler = function() {
        $(document).on('click', function(e) {
            ...
            if ($itemDiv.length > 0) {
                if (!jQuery.isEmptyObject(pageData.objsrc)) {
                    pageData.objsrc.removeClass('selected');
                }
