 ...
 * Description:国籍选择功能
 * 引用文件：
 * js: 'widget/nationality/jquery.nationality.js',
 * css: '../js/widget/nationality/css/nationality.css',
 * 调用方法：$('#nationid').selNation({});
 * 
 */
;
(function(factory) {
     ...
     * @type (string)
     */
    Plugin.pluginName = "selNation";

    /**
     * 插件缓存名称，插件通过 data 方法缓存在 dom 结构里，存储数据的名称
     * @type {string}
     */
    Plugin.dataName = "selDataNation";
    ...
    Plugin.prototype = {
        ...
        __createUlContainer: function() {
            var $listContainer = $('<div class="nationality-suggest-list-container">' +
                ' <div class="nationality-suggest-hint">输入中英文/代码搜索或&uarr;&darr;选择</div>' +
                '<ul class="nationality-suggest-list"></ul>' +
                '</div>');
            $listContainer.insertAfter(this.$element);
            this.options.list = $('.nationality-suggest-list');
        ...
        setBoxModel:function () {
            ...
            this.list.closest('.nationality-suggest-list-container').css('width',this.input.css('width'));
        },

        //功能描述：选项设置
        setOption: function(options) {
            if (!options || !options.input || !options.list) return;
        ...
        setEvent: function() {
            ...
            this.input.on("keyup input", function(event) {
        ...
        setListEvent: function() {
            var me = this;
            this.list.find(".nationality-suggest-item").unbind("click").on("click", function() {
